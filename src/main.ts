...
function status(device: Device) {
  ...
  if (device.remaining <= device.threshold) return ["low", "At refill threshold"];
...
function render(focusHeading = false) {
  document.title = pageTitle(); const path = location.pathname;
  const view = path === "/privacy" ? policy("Your data stays on this device", `<p>Dose Count Compass stores your device list and logs in your browser’s IndexedDB. It does not create an account or send your health data to us.</p><h2>Exports</h2><p>Exports are downloaded to your device only. You choose when to create or import a backup.</p><h2>Contact</h2><p>For a privacy question, contact the Param Factory through its product listing.</p>`) : path === "/terms" ? policy("Terms for using Dose Count Compass", `<p>This tool helps you keep a personal count. It is not medical advice and cannot confirm a device is usable.</p><h2>Your responsibility</h2><p>Check the physical device, its indicator, label, expiry date, and your clinician or pharmacist’s instructions. Keep a backup when your situation requires it.</p>`) : path === "/demo" || path === "/log" || routeIsDemo() ? dashboard() : home();
...
async function importBackup(event: Event) {
  ...
  try { const parsed = parseBackup(JSON.parse(await file.text())); if (!parsed) throw new Error("Invalid backup"); pendingImport = parsed; lastTriggerSelector = ".file-label"; render(); } catch { toast("That backup is not valid. Choose a Dose Count Compass JSON backup."); }
}
async function saveForm(event: SubmitEvent) {
  ...
  if (![total, remaining, threshold].every(Number.isFinite) || ![total, remaining, threshold].every(Number.isInteger) || total < 1 || remaining < 0 || remaining > total || threshold < 0 || threshold > total) { toast("Set whole-number counts: total at least 1, and refill threshold no higher than total."); return; }
...
function bind() {
  ...
  document.querySelector('[data-action="start-real"]')?.addEventListener("click", () => void navigate("/log")); document.querySelector('[data-action="reset-demo"]')?.addEventListener("click", () => void resetDemo());
